...
import { useAgeCases } from "@/features/dashboard/hooks/useEpidemiologicalData";
      ...
      <div className="space-y-1">
        ...
        {total > 0 && (
          <div className="text-xs text-gray-500 mt-1">
            Distribución: {((maleValue / total) * 100).toFixed(1)}% M /{" "}
            {((femaleValue / total) * 100).toFixed(1)}% F
          </div>
        )}
      </div>
    </div>
...
export const AgePyramidChart: React.FC<AgePyramidChartProps> = ({
  ...
  const { processedData, loading, error, refetch } = useAgeCases(filters);
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        {statistics && (
          <DemographicStats statistics={statistics} totalCases={totalCases} />
        )}
      </CardHeader>
      ...
      <CardContent>
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded-lg">
          ...
          <div className="text-sm text-gray-600 space-y-1">
            <p>
              • Grupo más afectado:{" "}
              <strong>{statistics?.mostAffectedAgeGroup}</strong>
            </p>
            <p>
              ...
              {statistics && (
                <>
                  <strong>
                    {(
                      (statistics.genderDistribution.male / totalCases) *
                      100
                    ).toFixed(1)}
                    % M
                  </strong>
                  ...
                  <strong>
                    {(
                      (statistics.genderDistribution.female / totalCases) *
                      100
                    ).toFixed(1)}
                    % F
                  </strong>
