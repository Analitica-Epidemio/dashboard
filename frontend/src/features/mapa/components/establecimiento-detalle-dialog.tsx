...
import { useEstablecimientoDetalle, type PersonaRelacionada } from "@/features/mapa/api";
import { Calendar, MapPin, Users, Activity, Building2, Link2 } from "lucide-react";
import { format } from "date-fns";
import { es } from "date-fns/locale";

interface EstablecimientoDetalleDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  idEstablecimiento: number | null;
}

export function EstablecimientoDetalleDialog({
  open,
  onOpenChange,
  idEstablecimiento,
}: EstablecimientoDetalleDialogProps) {
  const { data, isLoading } = useEstablecimientoDetalle(idEstablecimiento, { enabled: open });
        ...
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {isLoading ? (
              ...
              data?.data?.nombre || "Establecimiento"
            )}
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="flex-1 overflow-y-auto space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard
                ...
                value={data.data.total_personas}
                variant="default"
              />
              {data.data.codigo_refes && (
                <StatCard
                  icon={<Building2 className="h-4 w-4" />}
                  label="Código REFES"
                  value={data.data.codigo_refes}
                  variant="secondary"
                />
              )}
              {data.data.localidad_nombre && (
                <StatCard
              ...
              <StatCard
                ...
                label="Coordenadas"
                value={`${data.data.latitud.toFixed(4)}, ${data.data.longitud.toFixed(4)}`}
                variant="secondary"
              />
            </div>

            {/* Relaciones por tipo */}
            {data.data.relaciones_por_tipo && Object.keys(data.data.relaciones_por_tipo).length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(data.data.relaciones_por_tipo).map(([tipo, count]) => (
                    <Badge key={tipo} variant="outline" className="px-3 py-1">
                      {getTipoRelacionLabel(tipo)}: <span className="font-bold ml-1">{count as number}</span>
                    </Badge>
                  ))}
                </div>
              </div>
            ...
            {/* Eventos por tipo */}
            {data.data.eventos_por_tipo && Object.keys(data.data.eventos_por_tipo).length > 0 && (
              <div>
                <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(data.data.eventos_por_tipo).map(([tipo, count]) => (
                    <Badge key={tipo} variant="outline" className="px-3 py-1">
            ...
            <div>
              <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
                ...
                Personas Relacionadas ({data.data.personas?.length || 0})
              </h3>

              {!data.data.personas || data.data.personas.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  No hay personas relacionadas con este establecimiento
                </div>
              ) : (
                <div className="space-y-2 max-h-[400px] overflow-y-auto">
                  {data.data.personas.map((persona, idx) => (
