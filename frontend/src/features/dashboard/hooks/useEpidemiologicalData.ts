...
export function useSuicideAttemptData(filters?: EpidemiologicalFilters) {
  ...
  const processedData = useMemo(() => {
    ...
    const methodsData = Object.entries(demographics.methodsUsed).map(
      ([method, count]) => ({
        name: method,
        value: count,
      })
...
// Hook para datos KPI
export function useKPIData(filters?: EpidemiologicalFilters) {
  const chartState = useChartData(
    epidemiologicalApi.getKPIData,
    filters,
    true, // autoRefresh enabled for KPIs
  ...
  const processedData = useMemo(() => {
    ...
    const data = chartState.data as KPIData;
    ...
    return {
      ...data,
      percentageChanges: {
        totalCases: calculatePercentageChange(data.totalCases, data.previousPeriod.totalCases),
        newCases7Days: calculatePercentageChange(data.newCases7Days, data.previousPeriod.newCases7Days),
        incidenceRate: calculatePercentageChange(data.incidenceRate, data.previousPeriod.incidenceRate),
        mortalityRate: calculatePercentageChange(data.mortalityRate, data.previousPeriod.mortalityRate),
        affectedAreas: calculatePercentageChange(data.affectedAreas, data.previousPeriod.affectedAreas),
        averageResponseTime: calculatePercentageChange(data.averageResponseTime, data.previousPeriod.averageResponseTime),
      },
    };
  }, [chartState.data]);
...
// Hook para manejo de filtros epidemiológicos
export function useEpidemiologicalFilters(initialFilters?: EpidemiologicalFilters) {
  const [filters, setFilters] = useState<EpidemiologicalFilters>(
    initialFilters || { selectedGroupId: null, selectedEventId: null }
  );
  ...
  const resetFilters = useCallback(() => {
    setFilters(initialFilters || { selectedGroupId: null, selectedEventId: null });
  }, [initialFilters]);
