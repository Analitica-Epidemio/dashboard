...
import { EpidemiologicalChartsSection } from './EpidemiologicalChartsSection'
          ...
          <CardContent className="pt-6">
            <div className="space-y-2">
              <div className="text-sm text-muted-foreground">
                <span className="font-medium">Grupo seleccionado:</span> {selectedGroup.name}
              </div>
              {selectedEvent && (
                <div className="text-sm text-muted-foreground">
                  <span className="font-medium">Evento seleccionado:</span> {selectedEvent.name}
                  {selectedEvent.description && (
                    <span className="block text-xs mt-1">{selectedEvent.description}</span>
                  )}
                </div>
              )}
            </div>
      ...
      {selectedEvent && (
        <Tabs defaultValue="standard-charts" className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="standard-charts" className="flex items-center gap-2">
              <BarChart3 className="h-4 w-4" />
              Gráficos Estándar
            </TabsTrigger>
            <TabsTrigger value="epidemiological-analysis" className="flex items-center gap-2">
          ...
          <TabsContent value="standard-charts" className="space-y-4">
            <div className="flex items-center gap-2">
              <h2 className="text-2xl font-bold">Análisis de Datos - {selectedEvent.name}</h2>
            ...
            {chartDataError && (
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  {chartDataError.message || 'Error al cargar datos de gráficos'}
                </AlertDescription>
              </Alert>
            )}
