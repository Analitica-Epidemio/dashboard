...
import {
  ...
  useAceptarMapeosEstablecimientosBulk,
...
export function MapeoEstablecimientosPanel() {
  ...
  const aceptarBulkMutation = useAceptarMapeosEstablecimientosBulk();

  const establecimientos = data?.data?.items || [];
  const sinMapearCount = data?.data?.sin_mapear_count || 0;
  const eventosSinMapear = data?.data?.eventos_sin_mapear_count || 0;
  ...
  const handleAceptarTodasAltaConfianza = async () => {
    ...
    try {
      ...
      await aceptarBulkMutation.mutateAsync({
        body: { mapeos },
      });
                ...
                <div className="flex-1">
                  ...
                  {conAltaConfianza > 0 && (
                    <Button
                      ...
                      disabled={aceptarBulkMutation.isPending}
                      size="sm"
                      className="bg-orange-600 hover:bg-orange-700"
                    >
                      {aceptarBulkMutation.isPending ? (
                        <>
          ...
          {establecimientos.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              {establecimientos.map((estab) => {
                ...
                const isProcessing = crearMapeoMutation.isPending;
