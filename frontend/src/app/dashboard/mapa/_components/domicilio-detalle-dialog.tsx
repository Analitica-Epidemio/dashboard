...
interface DomicilioDetalleDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  idDomicilio: number | null;
}

export function DomicilioDetalleDialog({
  open,
  onOpenChange,
  idDomicilio,
}: DomicilioDetalleDialogProps) {
  const { data, isLoading } = useDomicilioDetalle(idDomicilio, open);
      ...
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-hidden flex flex-col">
        ...
        {isLoading ? (
          ...
          <div className="flex-1 overflow-y-auto space-y-6">
            ...
            {/* Casos por tipo */}
            {Object.keys(data.casos_por_tipo).length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(data.casos_por_tipo).map(([tipo, count]) => (
            ...
            <div>
              <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
                ...
                Lista de Casos ({data.casos.length})
              </h3>
