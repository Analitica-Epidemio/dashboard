...
function getEpidemiologicalWeek(date: Date): { week: number; year: number } {
  ...
  if (date < week1Sunday) {
    ...
    const weekNum = Math.floor((date - prevYearWeek1Sunday) / msPerWeek) + 1;
  ...
  const msPerWeek = 7 * 24 * 60 * 60 * 1000;
  const weekNum = Math.floor((date - week1Sunday) / msPerWeek) + 1;
  ...
  if (weekNum > 52) {
    ...
    const lastDayWeek = Math.floor((lastDayOfYear - week1Sunday) / msPerWeek) + 1;
...
export default function DemoPage() {
  useEffect(() => {
    ...
    testDates.forEach(date => {
      ...
      console.log(`${date.toLocaleDateString('es')} => SE${epi.week} del ${epi.year}`);
    });
  }, []);

  return (
    <div className="container mx-auto p-8">
      <h1 className="text-2xl font-bold mb-6">Calendario con Semanas Epidemiológicas</h1>
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div style={{ marginBottom: '1rem', padding: '1rem', background: '#f3f4f6', borderRadius: '0.5rem' }}>
          ...
          <ul style={{ fontSize: '0.875rem', marginTop: '0.5rem' }}>
            <li>• Primer sábado de enero 2025: 4 de enero (tiene 4 días en enero, así que termina SE1)</li>
        ...
        <DayPicker
          ...
          components={{
            WeekNumber: (props) => {
              ...
              console.log('Semana:', dates[0]?.toLocaleDateString(), '-', dates[6]?.toLocaleDateString(), '=> SE', epiData.week, 'del', epiData.year);

              return (
                <td className="rdp-week_number" style={{ padding: '0.25rem', textAlign: 'center', background: '#f9fafb' }}>
                  <div style={{ fontSize: '0.75rem', fontWeight: '600', color: '#1f2937' }}>
                    SE{epiData.week}
                    {epiData.year !== new Date().getFullYear() && (
                      <span style={{ display: 'block', fontSize: '0.625rem', color: '#6b7280' }}>
                        {epiData.year}
                      </span>
                    )}
                  </div>
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg">
          <h2 className="font-semibold text-blue-900 mb-2">Información sobre Semanas Epidemiológicas:</h2>
          <ul className="text-sm text-blue-800 space-y-1">
            <li>• Las semanas epidemiológicas van de domingo a sábado</li>
            <li>• La semana 1 termina el primer sábado de enero con al menos 4 días en enero</li>
            <li>• Se usan para estandarizar reportes de vigilancia epidemiológica</li>
            <li>• Siguen el estándar CDC/MMWR (Morbidity and Mortality Weekly Report)</li>
          </ul>
        </div>
      </div>
    </div>
