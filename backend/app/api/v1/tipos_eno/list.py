...
async def list_tipos_eno(
    ...
    ordenar_por: str = Query(
        "total_casos", description="Campo para ordenar: nombre, codigo, total_casos"
    ),
    ...
    try:
        ...
        casos_subquery = (
            select(
                col(CasoEpidemiologico.id_enfermedad),
                func.count(col(CasoEpidemiologico.id)).label("total_casos"),
            )
            .group_by(col(CasoEpidemiologico.id_enfermedad))
            .subquery()
        )
        ...
        # Aplicar filtros
        if nombre:
            query = query.where(col(Enfermedad.nombre).ilike(f"%{nombre}%"))

        # Filtrar por grupo usando subquery en la tabla de unión
        if grupo_id:
            query = query.where(
                col(Enfermedad.id).in_(
                    select(col(EnfermedadGrupo.id_enfermedad)).where(
                        col(EnfermedadGrupo.id_grupo) == grupo_id
                    )
                )
            )
        elif grupos:
            query = query.where(
                col(Enfermedad.id).in_(
                    select(col(EnfermedadGrupo.id_enfermedad)).where(
                        col(EnfermedadGrupo.id_grupo).in_(grupos)
                    )
                )
            )
        ...
        if ordenar_por == "nombre":
            query = query.order_by(order_func(col(Enfermedad.nombre)))
        elif ordenar_por == "slug":
            query = query.order_by(order_func(col(Enfermedad.slug)))
        elif ordenar_por == "total_casos":
            query = query.order_by(
                order_func(func.coalesce(casos_subquery.c.total_casos, 0))
            )
        else:
        ...
        count_query = select(func.count(col(Enfermedad.id).distinct()))
        if nombre:
            count_query = count_query.where(col(Enfermedad.nombre).ilike(f"%{nombre}%"))
        if grupo_id:
        ...
        return PaginatedResponse(
            ...
            links={
                "first": f"/api/v1/tiposEno?page=1&per_page={per_page}"
                if total_pages > 0
                else None,
                "prev": f"/api/v1/tiposEno?page={page - 1}&per_page={per_page}"
                if page > 1
                else None,
                "next": f"/api/v1/tiposEno?page={page + 1}&per_page={per_page}"
                if page < total_pages
                else None,
                "last": f"/api/v1/tiposEno?page={total_pages}&per_page={per_page}"
                if total_pages > 0
                else None,
            },
        )
