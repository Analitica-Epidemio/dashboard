...
class CasosMetrics(BaseModel):
    ...
    incidencia_100k: MetricValue = Field(
...
class CoberturaMetrics(BaseModel):
    ...
    areas_afectadas: MetricValue = Field(
        ..., description="Número de departamentos con casos"
    )
    nuevas_areas: int = Field(
...
class PerformanceMetrics(BaseModel):
    ...
    confianza_promedio: MetricValue = Field(
...
class AnalyticsResponse(BaseModel):
    ...
    cobertura: CoberturaMetrics = Field(
...
class CalculateChangesRequest(BaseModel):
    ...
    num_semanas: int = Field(
        default=4, description="Número de semanas hacia atrás", ge=1, le=52
    )
...
class CasoEpidemiologicoDetailsResponse(BaseModel):
    ...
    grupo_eno: GrupoDeEnfermedadesBasic = Field(
