...
class GrupoCasoEpidemiologico(BaseModel):
    ...
    descripcion: str | None = Field(None, description="Descripción del grupo")
...
class CasoEpidemiologicoDentroGrupo(BaseModel):
    """CasoEpidemiologico específico dentro de un grupo."""
    ...
    clasificaciones: list[str] | None = Field(
    ...
    ultimo_caso: str | None = Field(None, description="Fecha del último caso")
...
class ConfiguracionVisualizacion(BaseModel):
    ...
    descripcion: str | None = Field(None, description="Descripción del gráfico")
...
class GrupoCasoEpidemiologicoResponse(BaseModel):
    """Respuesta para un grupo de eventos."""

    grupo: GrupoCasoEpidemiologico
    eventos: list[CasoEpidemiologicoDentroGrupo]
...
class ListaGruposResponse(BaseModel):
    ...
    grupos: list[GrupoCasoEpidemiologico]
    total: int
...
class DatosVisualizacionRequest(BaseModel):
    ...
    fecha_desde: str | None = Field(None, description="Fecha desde (YYYY-MM-DD)")
    fecha_hasta: str | None = Field(None, description="Fecha hasta (YYYY-MM-DD)")
