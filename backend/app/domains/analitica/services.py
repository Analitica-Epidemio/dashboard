    ...
    async def get_grupo_detalle(
        ...
        if grupo_config["tipo"] == "grupo":
            for evento_nombre in grupo_config.get("eventos", []):
                tipo_eno_evento = (
                    self.session.query(Enfermedad)
                    .filter(Enfermedad.nombre == evento_nombre)
                    .first()
                )
            ...
            # CasoEpidemiologico simple
            tipo_eno = (
                self.session.query(Enfermedad)
                .filter(Enfermedad.nombre == grupo_nombre)
                .first()
            )
            if tipo_eno:
        ...
        return GrupoCasoEpidemiologicoResponse(
