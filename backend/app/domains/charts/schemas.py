...
class ConjuntoDatos(BaseModel):
    ...
    tipo: Optional[Literal["area", "line"]] = Field(None, serialization_alias="type")  # Para area charts con líneas
...
class ConfiguracionGraficoTorta(ConfiguracionGraficoBase):
    ...
    mostrar_porcentajes: Optional[bool] = Field(True, serialization_alias="showPercentages")
    radio_interno: Optional[int] = Field(0, serialization_alias="innerRadius")  # For donut charts
...
class WrapperConfiguracionGraficoPiramide(BaseModel):
    ...
    tipo: Literal["d3_pyramid"] = Field("d3_pyramid", serialization_alias="type")
    configuracion: ConfiguracionGraficoPiramide = Field(..., serialization_alias="config")
...
class ErrorGrafico(BaseModel):
    ...
    codigo: str = Field(..., serialization_alias="code")  # Código de error (ej: "INSUFFICIENT_HISTORICAL_DATA")
    titulo: str = Field(..., serialization_alias="title")  # Título corto para mostrar
    mensaje: str = Field(..., serialization_alias="message")  # Mensaje descriptivo para el usuario
    detalles: Optional[Dict[str, Any]] = Field(None, serialization_alias="details")  # Detalles técnicos opcionales
    sugerencia: Optional[str] = Field(None, serialization_alias="suggestion")  # Sugerencia de cómo resolver
