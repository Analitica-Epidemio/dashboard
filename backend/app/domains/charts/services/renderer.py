...
class ChartRenderer:
    ...
    def renderizar_a_bytes(
        self, spec: EspecificacionGraficoUniversal, dpi: int = 300
    ) -> bytes:
        """
    ...
    def _get_config_value(self, config: Any, key: str, default: Any = None) -> Any:
        """Helper para obtener valores de configuración (dict o objeto)."""
    ...
    def _renderizar_grafico_linea(
        ...
        for dataset in datos.datos.conjuntos_datos:
            ...
            mostrar_puntos = self._get_config_value(config, "mostrar_puntos")
        ...
        if self._get_config_value(config, "mostrar_grilla"):
    ...
    def _renderizar_grafico_barra(
        ...
        for i, dataset in enumerate(datos.datos.conjuntos_datos):
            offset = (
                i - len(datos.datos.conjuntos_datos) / 2
            ) * width_bar + width_bar / 2
            color = (
                self._normalizar_color(dataset.color)
                if dataset.color
                else colors_palette[i % len(colors_palette)]
            )
            ax.bar(
                x + offset,
                dataset.datos,
        ...
        if (
            self._get_config_value(config, "mostrar_leyenda")
            and len(datos.datos.conjuntos_datos) > 1
        ):
            legend = ax.legend(frameon=True, fancybox=True, shadow=False)
            legend.get_frame().set_alpha(0.95)
        if self._get_config_value(config, "mostrar_grilla"):
    ...
    def _renderizar_grafico_area(
        ...
        def extract_values(dataset_data: list[Any]) -> list[float]:
            ...
            for val in dataset_data:
                if isinstance(val, dict) and "parsedValue" in val:
                    values.append(float(val["parsedValue"]))
                else:
        ...
        for i, dataset in enumerate(datos.datos.conjuntos_datos):
            ...
            if dataset.tipo == "line":
                ...
                opacidad = self._get_config_value(config, "opacidad_relleno")
                alpha = opacidad if opacidad is not None else 0.2
                ax.fill_between(
                    x,
                    values,
                    alpha=alpha,
                ...
                # Borde del área
                ax.plot(x, values, color=color, linewidth=1.5, alpha=0.6)
        ...
        if num_labels > 0:
            ...
            if len(tick_positions) == 0:
                tick_positions = [0]
            tick_labels = [
                datos.datos.etiquetas[i]
                for i in tick_positions
                if i < len(datos.datos.etiquetas)
            ]
    ...
    def _renderizar_grafico_torta(
        ...
        alto_val = self._get_config_value(config, "alto")
        alto = alto_val / 100 if alto_val else 6
        ...
        # ax.pie con autopct retorna 3 elementos: (wedges, texts, autotexts)
        # Usamos _ para indicar que no usamos los valores directamente
        _ = pie_result  # Desempaquetar para satisfacer el type checker
    ...
    def _renderizar_grafico_piramide(
        self, spec: EspecificacionGraficoUniversal, dpi: int
    ) -> bytes:
    ...
    def _renderizar_grafico_mapa(
        ...
        svg_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "assets", "chubut.svg"
        )
        ...
        try:
            ...
            if drawing:
                ...
                # Título
                ax.set_title(
                    spec.titulo, fontsize=16, fontweight="600", color="#1F2937", pad=15
                )
    ...
    def renderizar_tabla_departamentos(
        self, datos: WrapperDatosGraficoMapa, titulo: str = "", dpi: int = 300
    ) -> bytes:
        """
        ...
        table = ax.table(
            ...
            bbox=Bbox.from_bounds(0.05, 0.1, 0.9, 0.85),
        )
