...
class CasoGrupoEnfermedad(BaseModel, table=True):
    ...
    caso: "CasoEpidemiologico" = Relationship(back_populates="caso_grupos")
    grupo: "GrupoDeEnfermedades" = Relationship(back_populates="casos_en_grupo")
...
class AntecedenteEpidemiologico(BaseModel, table=True):
    ...
    id_snvs_antecedente_epidemio: Optional[int] = Field(
        None,
        description="ID del antecedente en SNVS"
    )
    ...
    antecedentes_casos: List["AntecedentesCasoEpidemiologico"] = Relationship(
...
class CasoEpidemiologico(BaseModel, table=True):
    ...
    fecha_minima_caso_semana_epi: int = Field(
        ...,
        index=True,
        description="Semana epidemiológica de fecha_minima_caso (1-53)"
    )
    fecha_minima_caso_anio_epi: int = Field(
        ...,
        index=True,
        description="Año epidemiológico de fecha_minima_caso"
    )
    ...
    es_caso_sintomatico: Optional[bool] = Field(
        None,
        description="True si el caso presenta síntomas"
    )
    fecha_apertura_caso: Optional[date] = Field(
        None,
        description="Fecha de apertura del caso en SNVS"
    )
    ...
    fecha_primera_consulta: Optional[date] = Field(
        None,
        description="Fecha de primera consulta médica"
    )
    anio_epidemiologico_consulta: Optional[int] = Field(
        None,
        description="Año epidemiológico de consulta"
    )
    semana_epidemiologica_consulta: Optional[int] = Field(
        None,
        description="Semana epidemiológica de consulta"
    )
    semana_minima_calculada: Optional[int] = Field(
        None,
        description="Semana mínima calculada del caso"
    )
    anio_evento: Optional[int] = Field(
        None,
        description="Año calendario del evento"
    )
    ...
    id_origen: Optional[str] = Field(
        None,
        max_length=200,
        description="Sistema de origen del registro"
    )
    semana_epidemiologica_sintomas: Optional[int] = Field(
        None,
        description="Semana epidemiológica de inicio de síntomas"
    )
    semana_epidemiologica_muestra: Optional[int] = Field(
        None,
        description="Semana epidemiológica de toma de muestra"
    )
    ...
    id_animal: Optional[int] = Field(
        ...
        description="ID del animal (si es caso animal)"
    )
    ...
    requiere_revision_especie: Optional[bool] = Field(
        False,
        description="True si requiere revisión de especie (animal)"
    )
    datos_originales_csv: Optional[Dict] = Field(
        None,
        sa_column=Column(JSON),
        description="Datos originales del CSV importado"
    )
    ...
    clasificacion_manual: Optional[str] = Field(
        None,
        max_length=500,
        description="Clasificación original del SNVS"
    )
    ...
    metadata_extraida: Optional[Dict] = Field(
        None,
        sa_column=Column(JSON),
        description="Metadata extraída del procesamiento"
    )
    ...
    enfermedad: "Enfermedad" = Relationship(back_populates="casos")
    caso_grupos: List["CasoGrupoEnfermedad"] = Relationship(back_populates="caso")
    ciudadano: Optional["Ciudadano"] = Relationship(back_populates="casos")
    animal: Optional["Animal"] = Relationship(back_populates="casos")
    estrategia_aplicada: Optional["EstrategiaClasificacion"] = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[CasoEpidemiologico.id_estrategia_aplicada]"}
    )
    establecimiento_consulta: Optional["Establecimiento"] = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[CasoEpidemiologico.id_establecimiento_consulta]"}
    )
    establecimiento_notificacion: Optional["Establecimiento"] = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[CasoEpidemiologico.id_establecimiento_notificacion]"}
    )
    establecimiento_carga: Optional["Establecimiento"] = Relationship(
        sa_relationship_kwargs={"foreign_keys": "[CasoEpidemiologico.id_establecimiento_carga]"}
    )
    sintomas: List["DetalleCasoSintomas"] = Relationship(back_populates="caso")
    muestras: List["MuestraCasoEpidemiologico"] = Relationship(back_populates="caso")
    antecedentes: List["AntecedentesCasoEpidemiologico"] = Relationship(back_populates="caso")
    vacunas: List["VacunasCiudadano"] = Relationship(back_populates="caso")
    diagnosticos: List["DiagnosticoCasoEpidemiologico"] = Relationship(back_populates="caso")
    internaciones: List["InternacionCasoEpidemiologico"] = Relationship(back_populates="caso")
    tratamientos: List["TratamientoCasoEpidemiologico"] = Relationship(back_populates="caso")
    investigaciones: List["InvestigacionCasoEpidemiologico"] = Relationship(back_populates="caso")
    contactos: List["ContactosNotificacion"] = Relationship(back_populates="caso")
    ambitos_concurrencia: List["AmbitosConcurrenciaCaso"] = Relationship(back_populates="caso")
    domicilio: Optional["Domicilio"] = Relationship(back_populates="casos")
    agentes_detectados: List["CasoAgente"] = Relationship(
        back_populates="caso",
...
class DetalleCasoSintomas(BaseModel, table=True):
    ...
    __table_args__ = (UniqueConstraint("id_caso", "id_sintoma", name="uq_caso_sintoma"),)

    semana_epidemiologica_aparicion_sintoma: Optional[int] = Field(
        None,
        description="Semana epidemiológica de aparición del síntoma"
    )
    ...
    anio_epidemiologico_sintoma: Optional[int] = Field(
        None,
        description="Año epidemiológico del síntoma"
    )
    ...
    id_sintoma: int = Field(
        foreign_key="sintoma.id",
        description="ID del síntoma"
    )

    caso: "CasoEpidemiologico" = Relationship(back_populates="sintomas")
    sintoma: "Sintoma" = Relationship(back_populates="detalle_casos")
...
class AntecedentesCasoEpidemiologico(BaseModel, table=True):
    ...
    __table_args__ = (
        UniqueConstraint("id_caso", "id_antecedente_epidemiologico", name="uq_caso_antecedente"),
    )

    fecha_antecedente_epidemiologico: Optional[date] = Field(
        None,
        description="Fecha del antecedente (si aplica)"
    )
    ...
    id_antecedente_epidemiologico: int = Field(
        foreign_key="antecedente_epidemiologico.id",
        description="ID del antecedente"
    )

    caso: "CasoEpidemiologico" = Relationship(back_populates="antecedentes")
