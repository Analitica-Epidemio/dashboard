...
from app.domains.vigilancia_nominal.models.atencion import TratamientoCasoEpidemiologico
...
class TratamientosProcessor(BulkProcessorBase):
    ...
    def upsert_tratamientos_eventos(self, df: pl.DataFrame) -> BulkOperationResult:
        ...
        tratamientos_df = df.filter(
            (
                pl.col(Columns.TRATAMIENTO_2.name).is_not_null()
                if Columns.TRATAMIENTO_2.name in df.columns
                else pl.lit(False)
            )
            | (
                pl.col(Columns.FECHA_INICIO_TRAT.name).is_not_null()
                if Columns.FECHA_INICIO_TRAT.name in df.columns
                else pl.lit(False)
            )
            | (
                pl.col(Columns.FECHA_FIN_TRAT.name).is_not_null()
                if Columns.FECHA_FIN_TRAT.name in df.columns
                else pl.lit(False)
            )
            | (
                pl.col(Columns.RESULTADO_TRATAMIENTO.name).is_not_null()
                if Columns.RESULTADO_TRATAMIENTO.name in df.columns
                else pl.lit(False)
            )
        )
        ...
        if tratamientos_prepared.height == 0:
            self.logger.info(
                "No hay tratamientos válidos después de filtrado y deduplicación"
            )
            return BulkOperationResult(0, 0, 0, [], 0.0)

        self.logger.info(
            f"Tratamientos después de deduplicación: {tratamientos_prepared.height}"
        )
        ...
        # PostgreSQL UPSERT
        table = SQLModel.metadata.tables[TratamientoCasoEpidemiologico.__tablename__]
