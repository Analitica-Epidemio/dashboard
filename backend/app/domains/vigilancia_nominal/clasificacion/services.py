...
class EventClassificationService:
    ...
    async def classify_events(
        ...
        """
        # Obtener estrategia
        strategy = await self._get_strategy(id_enfermedad, use_cache)
    ...
    async def _get_strategy(
        ...
        strategy = await self.strategy_repo.get_by_tipo_enfermedad(id_enfermedad)

        if strategy and use_cache:
            self._cache[id_enfermedad] = strategy
    ...
    async def test_classification_legacy(
        ...
        # Clasificar
        result_df = await self.classify_events(df, id_enfermedad, use_cache=False)
...
class StrategyValidationService:
    ...
    async def validate_strategy(self, strategy: EstrategiaClasificacion) -> List[str]:
        ...
        overlapping_strategies = await self.strategy_repo.check_date_overlap(
            id_enfermedad=strategy.id_enfermedad,
