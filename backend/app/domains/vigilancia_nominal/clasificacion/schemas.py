...
class EstrategiaClasificacionBase(BaseModel):
    """Base DTO para estrategias."""

    name: str = Field(
        ..., min_length=1, max_length=255, description="Nombre de la estrategia"
    )
    id_enfermedad: int = Field(..., description="ID del tipo de evento epidemiológico")
...
class EstrategiaClasificacionResponse(EstrategiaClasificacionBase):
...
class BatchClassificationRequest(BaseModel):
    ...
    id_enfermedad: int = Field(..., description="ID del tipo de evento")
