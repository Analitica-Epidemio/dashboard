...
class ProcessingJob(SQLModel, table=True):
    ...
    # Progreso
    progress_percentage: int = Field(default=0, description="Porcentaje de completado (0-100)")
    current_step: Optional[str] = Field(default=None, description="Paso actual de procesamiento")
    ...
    file_path: Optional[str] = Field(default=None, description="Ruta del archivo procesado")
    file_size: Optional[int] = Field(default=None, description="Tamaño del archivo en bytes")
    sheet_name: Optional[str] = Field(default=None, description="Nombre de la hoja procesada")
    
    # Resultados
    total_rows: Optional[int] = Field(default=None, description="Total de filas procesadas")
    columns: Optional[List[str]] = Field(default=None, sa_column=Column(JSON), description="Columnas del archivo")
    validation_errors: Optional[List[str]] = Field(default=None, sa_column=Column(JSON), description="Errores de validación")
    
    # Metadata y tracking
    job_metadata: Optional[Dict[str, Any]] = Field(default=None, sa_column=Column(JSON), description="Metadata adicional")
    error_message: Optional[str] = Field(default=None, description="Mensaje de error si falla")
    error_traceback: Optional[str] = Field(default=None, description="Traceback completo del error")
    
    # Timestamps
    created_at: datetime = Field(default_factory=datetime.now, description="Momento de creación")
    started_at: Optional[datetime] = Field(default=None, description="Momento de inicio")
    completed_at: Optional[datetime] = Field(default=None, description="Momento de finalización")
    updated_at: datetime = Field(default_factory=datetime.now, description="Última actualización")
    
    # Celery específico
    celery_task_id: Optional[str] = Field(default=None, description="ID de la task de Celery")
    
    # Usuario
    created_by: Optional[str] = Field(default=None, description="Usuario que creó el trabajo")
