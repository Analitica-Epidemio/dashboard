...
class PostgreSQLProcessingJobRepository(ProcessingJobRepositoryInterface):
    ...
    async def create(self, job: ProcessingJob) -> ProcessingJob:
        ...
        """
        logger.info(f"💾 Repository.create() called - job_type: {job.job_type}, original_filename: {job.original_filename}")
        try:
    ...
    async def get_jobs_by_status(self, statuses: List[JobStatus]) -> List[ProcessingJob]:
    ...
    async def cleanup_old_jobs(self, days_old: int = 7) -> int:
        ...
        try:
            ...
            with self.session_factory(engine) as session:
                ...
                logger.info(f"Limpieza completada: {count} trabajos antiguos eliminados")
                return count
        ...
        try:
            with self.session_factory(engine) as session:
                ...
                total_in_progress = session.exec(
                    select(ProcessingJob).where(ProcessingJob.status == JobStatus.IN_PROGRESS)
                ).all()
                ...
                return {
                    "total": len(total_pending) + len(total_in_progress) + len(total_completed) + len(total_failed),
