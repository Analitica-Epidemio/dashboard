...
def agregar_mappings(data):
    ...
    for snvs_code, mapping_data in HOSPITALES_SCORE_100.items():
        if snvs_code in data["mapping"]:
            ...
            print(f"    → IGN: {mapping_data['nombre_ign']} (REFES: {mapping_data['codigo_refes']})")
            print(f"    → Score: {mapping_data['score']} | Similitud: {mapping_data['similitud_nombre']}%")
...
def main():
    ...
    if nuevos > 0:
        ...
        print("  • HOSPITAL ZONAL ESQUEL: código 8862 (agregado) y 13571 (no procesado)")
        print("  • HOSPITAL RURAL GUALJAINA: código 8808 (agregado) y 13573 (no procesado)")
        print()
