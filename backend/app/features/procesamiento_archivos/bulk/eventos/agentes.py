...
class AgentesExtractor:
    ...
    def _load_agentes_by_codigo(self) -> Dict[str, int]:
        ...
        stmt = select(AgenteEtiologico.id, AgenteEtiologico.codigo).where(
            AgenteEtiologico.activo == True  # noqa: E712
        )
        result = self.context.session.execute(stmt)
        rows = result.fetchall()

        self._agentes_by_codigo = {row.codigo: row.id for row in rows}
        return self._agentes_by_codigo
    ...
    def upsert_agentes_eventos(
        ...
        self.logger.info(f"  Eventos CSV: {len(eventos_csv)}, reglas: {len(self._reglas_lower)}, match: {len(eventos_con_match)}")
        ...
        # Bulk insert/update
        try:
            stmt = pg_insert(EventoAgente.__table__).values(unique_agentes)
            stmt = stmt.on_conflict_do_update(
                constraint="uq_evento_agente",
                set_={
