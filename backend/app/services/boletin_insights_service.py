...
class BoletinInsightsService:
    ...
    async def generate_distribucion_edad_insight(
        self,
        evento_id: int,
        fecha_inicio: date,
        fecha_fin: date,
        ...
        if not evento_nombre:
            stmt = select(TipoEno.nombre).where(TipoEno.id == evento_id)
            result = await self.db.execute(stmt)
            row = result.scalar_one_or_none()
            evento_nombre = row if row else f"evento {evento_id}"

        # Configuración de grupos de edad
        age_groups_config = get_age_groups_config("standard")
    ...
    async def generate_distribucion_geografica_insight(
        self,
        evento_id: int,
        fecha_inicio: date,
        fecha_fin: date,
        ...
        if departamentos:
            dept_mayor = departamentos[0]
            texto = (
                f"El departamento con mayor número de casos es {dept_mayor['nombre']} "
                f"con {dept_mayor['casos']} casos ({dept_mayor['porcentaje']}%)"
            )
        ...
        return {
            "texto": texto,
            "total_casos": total_casos,
            "departamento_mayor": dept_mayor,
            "departamentos": departamentos
        }

    async def generate_tendencia_insight(
        self,
        evento_id: int,
        semana_actual: int,
        anio: int,
        ...
        stmt_actual = (
            ...
            .where(
                and_(
                    ...
                    Evento.fecha_minima_evento >= fecha_inicio_actual,
                    Evento.fecha_minima_evento <= fecha_fin_actual
                )
            )
        )
    ...
    async def generate_resumen_insight(
        self,
        evento_id: int,
        fecha_inicio: date,
        fecha_fin: date,
        ...
        insight_edad = await self.generate_distribucion_edad_insight(
            evento_id, fecha_inicio, fecha_fin, evento_nombre
        )
        insight_geo = await self.generate_distribucion_geografica_insight(
            evento_id, fecha_inicio, fecha_fin, evento_nombre
        )
