...
Revision ID: 4027c7162835
Revises: 
Create Date: 2025-08-28 06:22:07.311914
...
def upgrade() -> None:
    ...
    sa.Column('casos_especiales', sa.Text(), nullable=True),
    sa.Column('grupo_evento', sqlmodel.sql.sqltypes.AutoString(length=100), nullable=True),
    sa.Column('usa_provincia_carga', sa.Boolean(), nullable=False),
    sa.Column('eventos_relacionados', sa.JSON(), nullable=True),
    sa.Column('graficos_disponibles', sa.JSON(), nullable=True),
    ...
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('tipo_eno_id')
    )
    ...
    op.create_index(op.f('ix_muestra_evento_id_snvs_muestra'), 'muestra_evento', ['id_snvs_muestra'], unique=True)
